...
export class InspectionService {
    ...
    private async publishStage(stage: INSPECTION_STAGE): Promise<void> {
        const list = this.inspectionRepo.getTeamsByStage(stage as INSPECTION_STAGE);
        await this.publisher.broadcast(`inspection/stage/${stage}`, { teams: list });
    }

    private async publishTeam(teamNumber: string, stage: INSPECTION_STAGE) {
        await this.publisher.broadcast(`inspection/team/${teamNumber}`, stage);
    }

    async markCheckinStage(team: string, stage: INSPECTION_STAGE) {
    ...
    async markMetOrNot(team: string, criteria: number, isMet: boolean) {
        const previous = this.inspectionRepo.getStage(team);
    ...
    async getChecklist() {
        return await this.inspectionRepo.getInspectionCriteria();
    }

    setEventStage(stage: EVENT_STAGE) {
        this.eventStage = stage;
    }

    private async handleChange(team: string, prev: INSPECTION_STAGE, now: INSPECTION_STAGE) {
        if (prev !== now) {
            this.logger.log(`Team ${team} changed from ${prev} to ${now}`);
            Promise.all([
