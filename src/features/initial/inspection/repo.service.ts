...
export class InspectionDatabase {
    ...
    async markCheckinStage(team: string, status: INSPECTION_STAGE): Promise<INSPECTION_STAGE> {
        await this.prisma.checkIn.upsert({ where: { teamNumber: team }, update: { status: status }, create: { teamNumber: team, status: status } });
    ...
    async getInspectionCriteria(): Promise<{[group: string]: Array<{text: string, uuid: number}>}> {
        const output = {}
    ...
    async initialLoad(team: string): Promise<INSPECTION_STAGE> {
        ...
        const teamRollup: InspectionRollup = {id: team, team: team, status: stage}
