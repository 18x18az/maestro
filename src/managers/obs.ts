...
export namespace Studio {
  ...
  async function getSceneInfo (sceneName: string): Promise<StandardScene> {
    const timerId = await getSceneItemId(sceneName, 'Timer')
    const audienceId = await getSceneItemId(sceneName, 'Audience')
  ...
  export async function setField (field: number): Promise<boolean> {
    return await setPreviewScene(field, OVERLAY.TIMER)
  }
